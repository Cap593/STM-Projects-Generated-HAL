...
void MCP2515_LoopbackInit(void)
{
    MCP2515_Reset();

    /*for (int i = 0; i < 20; i++)
    {
        val = MCP2515_Read(CANSTAT);
	    sprintf(msgMCP2515,"CANSTAT raw: 0x%02X\r\n", val);
	    HAL_UART_Transmit(&huart2,(uint8_t*)msgMCP2515,strlen(msgMCP2515),100);
        HAL_Delay(100);
    }*/
    ...
    // Bit timing: 500kbps @ 8MHz
    MCP2515_Write(CNF1, 0x00);
    MCP2515_Write(CNF2, 0xD1);
    MCP2515_Write(CNF3, 0x01);

    // Accept all messages
    MCP2515_Write(0x20, 0x00);
    MCP2515_Write(0x21, 0x00);
    MCP2515_Write(0x24, 0x00);
    MCP2515_Write(0x25, 0x00);
    ...
    while ((MCP2515_Read(CANSTAT) & 0xE0) != 0x40);
