...
int main(void)
{
  ...
  while (1)
  {
	...
	MCP2515_Send(0x123, txData, 2);
	HAL_Delay(100);

	if(MCP2515_Receive(&rxId, rxData, &rxLen))
	{
	    sprintf(msgMCP,
	        "Loopback OK ID:0x%03X DLC:%d Data:%02X %02X\r\n",
	        rxId,
	        rxLen,
	        rxData[0],
	        rxData[1]);
	...
	{
	    sprintf(msgMCP,"Loopback FAILED\r\n");
	    HAL_UART_Transmit(&huart2,
	        (uint8_t*)msgMCP,
	        strlen(msgMCP),
	        100);
