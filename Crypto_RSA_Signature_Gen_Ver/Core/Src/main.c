...
static int rsa_imported_pem_sign_verify_error_demo(void)
{
    int ret = -1;
    ...
    uint8_t message[] = "Hello RSA Signature";
    uint8_t tampered_message[] = "Hello RSA SignaturE";
    uint8_t hash[32];
    uint8_t signature[512];
    size_t sig_len = 0;
    ...
    ret = mbedtls_sha256_ret(tampered_message, strlen((const char *)tampered_message), hash, 0);
    if (ret != 0)
    {
        UART_Print("SHA256 for verify failed\r\n");
    }
...
int main(void)
{
  ...
  if (rsa_imported_pem_sign_verify_error_demo() != 0)
  {
	UART_Print("RSA DEMO OK\r\n");
  }
  else
