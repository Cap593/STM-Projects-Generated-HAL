...
MCP2515_Handle_t mcpA = {&hspi1, GPIOA, GPIO_PIN_4};
MCP2515_Handle_t mcpB = {&hspi1, GPIOB, GPIO_PIN_0};
...
int main(void)
{
  ...
  while (1)
  {
	...
	MCP2515_Send(&mcpA, 0x123, data, 3);

	if (MCP2515_Receive(&mcpB, &rxId, rxData, &rxLen))
	{
	  sprintf(msgMCP,"RX OK\r\n");
	  HAL_UART_Transmit(&huart2, msgMCP, sizeof(msgMCP), HAL_MAX_DELAY);
	}
